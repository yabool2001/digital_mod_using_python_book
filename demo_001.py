...
( t , x_n ) = gen_signal.wave ( 1 , 10 , 2000.0 , f_s , N , 1/2*np.pi , 10 , verbose = False ) #fi phase = 1 / 3 * np.pi #phase shift in radians
( t , x_n2 ) = gen_signal.wave ( 0 , 1 , 4000.0 , f_s , N , np.pi , 10 , verbose = False ) #fi phase = 1 / 3 * np.pi #phase shift in radians
x_n = x_n + x_n2
plot_signal ( t , x_n )
...
ft.plot_dft ( m_shift_freq , X_m_shift_mag , X_m_shift_phi )