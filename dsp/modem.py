...
class Modem:
    ...
    def modulate ( self , inputSymbols ) :
        """
        Modulate a vector of input symbols (numpy array format) using the chosen modem. Input symbols take integer values in the range 0 to M-1.
        """
    ...
    def iqDetector ( self , receivedSyms ) :
        ...
        Note: MPAM/BPSK are one dimensional modulations. The same function can be applied for these modulations since quadrature is zero (Q=0). The function computes the pair-wise Euclidean distance of each point in the received vector against every point in the reference constellation. It then returns the symbols from the reference constellation that provide the minimum Euclidean distance. Parameters:
